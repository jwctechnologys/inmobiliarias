...
const ReporteNovedadesArrendatario = () => {
  ...
  // Cargar reportes cuando userId esté disponible
  useEffect(() => {
    if (!userId) return; // No hacer nada si no hay un userId

    const fetchReportes = async () => {
      setLoading(true);

      try {
        const arrendatarioId = userId; // Usar el ID del usuario como arrendatarioId
        console.log('Verificando arrendatarioId:', arrendatarioId);
  ...
  const handleEnviar = (reporteId) => {
    const estaResueltoArrendatario = resueltos[reporteId]; // Asumiendo que 'resueltos' es un objeto con el reporteId como clave
    ...
    <div>
      ...
      {reportes.length === 0 ? (
        ...
        <table>
          ...
          <tbody>
            {reportes
            .filter((reporte) => !reporte.estaResueltoArrendatario)
            .map((reporte) => (
              <tr key={reporte.id}>
...
export default ReporteNovedadesArrendatario;
