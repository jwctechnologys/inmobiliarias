...
const OtroSiArrendatarioImprimir = () => {
  const { id } = useParams();
  ...
  const handleAceptarContrato = async () => {
    const contratoIdd = contratoOtrosi[0]?.contrato
          ...
          (user.groups[0] === 'arrendatario' && (
            <>
              ...
              <div className="acciones-contrato">
                {user.groups[0] === 'arrendatario' && (<>
                  {!contratoOtrosi[0].aceptaOtroSiArrendatario && (
                    <>
...
export default OtroSiArrendatarioImprimir;