...
const RegisterPropietario = () => {
    ...
    const [group, setGroup] = useState("propietario");
    ...
    const checkPasswordStrength = (pass) => {
        ...
        // Calcular puntuación (cada requisito vale 1 punto)
    ...
    const handleRegister = async (e) => {
        ...
        // Crear payload limpiando campos vacíos (convertir a null o undefined)
        const payload = {
            username,
            first_name,
            last_name,
        ...
        if (cuentaDaviplata && cuentaDaviplata.trim() !== '') {
            payload.cuentaDaviplata = cuentaDaviplata;
        }
        
        if (cuentaNequi && cuentaNequi.trim() !== '') {
            payload.cuentaNequi = cuentaNequi;
        }
        
        if (CuentaBancolombia && CuentaBancolombia.trim() !== '') {
            payload.CuentaBancolombia = CuentaBancolombia;
        }
        ...
        if (celularDos && celularDos.trim() !== '') {
            payload.celularDos = celularDos;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(`Error: ${data.error || 'Error al crear el propietario'}`);
            }
        } catch (error) {
    ...
    const inputClassName = "w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 bg-white";
    ...
    const selectClassName = "w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 bg-white";
    const labelClassName = "block text-sm font-medium text-gray-700 mb-1";
    const sectionTitleClassName = "text-lg font-semibold text-gray-800 mb-4 pb-2 border-b-2 border-green-200";
                    ...
                    <form onSubmit={handleRegister} className="space-y-6">
                        ...
                        <div className="pt-4">
                            <button
                                ...
                                className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-4 rounded-lg shadow-md hover:shadow-lg transform transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                            >
                                {cargando ? (
