...
const [formData, setFormData] = useState({
  ...
  const [clausulas, setClausulas] = useState([
    ...
    {
      id: 3,
      texto: 'Cláusula 3: Termino del contrato: se estipula por el término de unos (3), meses, inicia el 24 de julio de dos mil veinticuatro  (2024) y finaliza el 03 de octubre  de dos mil veinticuatro (2024).',
      selected: true,
    },
    {
      id: 4,
    ...
    {
      id: 17,
      texto: 'Cláusula 17: Gestión de cobro:</strong> en caso de retardo en el pago del canon mensual el arrendatario reconocerá a la arrendadora los gastos que por gestión de cobro se generen, sin que este pago signifique convalidación ó consentimiento en la mora por parte de la arrendadora y sin menoscabo de las acciones que interpongan ó deba instaurar la misma.',
      selected: true,
    },
    {
      id: 18,
  ...
  useEffect(() => {
    ...
    // Cargar coarrendatarios
    fetch(`${import.meta.env.VITE_BASE_URL}/api/coarrendatario/`)
      .then((response) => response.json())
      .then((data) => {
        //console.log('Datos de coarrendatarios:', data); // Agrega esto
        setCoarrendatarios(data);
    })
      .catch((error) => console.error('Error al cargar coarrendatarios:', error));
...
// Handler for updating userArrendatario with an ID
const handleUserArrendatarioChange = (id) => {
    setFormData((prevData) => ({
        ...prevData,
        userArrendatario: { id },  // Update userArrendatario with an ID
    }));
};
...
useEffect(() => {
  if (formData.inmueble) {
      //console.log('formData.inmueble:', formData.inmueble);  // Verifica que formData.inmueble tenga el id correcto
      //console.log('inmuebles:', inmuebles);  // Verifica que inmuebles esté correctamente cargado

      // Convertir formData.inmueble.id a número para compararlo con el id del inmueble
          ...
          setClausulas(prevClausulas => 
              prevClausulas.map(clausula => {
                  if (clausula.id === 1) {
                      return {
                          ...clausula,
                          texto: `Cláusula 1: Objeto del contrato: mediante el presente contrato, el arrendador concede al arrendatario el goce del local con vivienda ubicado en ${inmueble.direccion}.`
                      };
                  }
                  return clausula;
              })
...
useEffect(() => {
  ...
  if (formData.diaHInicioPago && formData.diaFinPago && formData.inmueble && inmuebleSeleccionado && administradores) {
      //console.log('Actualizando cláusula 4');
      setClausulas(prevClausulas =>
          prevClausulas.map(clausula => {
              if (clausula.id === 4) {
                  return {
                      ...clausula,
                      texto: `Cláusula 4: Remuneración mensual o canon de arrendamiento: del (${formData.diaHInicioPago}) al (${formData.diaFinPago}) de cada mes, EL ARRENDATARIA se obliga a pagar ALA ARRENDADORA la suma de ${inmuebleSeleccionado.canonmensual} pesos mc ($${inmuebleSeleccionado.canonmensual}), mediante consignación bancaria a las cuentas habilitadas para Consignación: Cuenta Daviplata N. ${administradores.cuentaDaviplata} Y/o cuenta Nequi N. ${administradores.cuentaNequi}  y Cuenta de ahorros Bancolombia N. xxxxxxxxxx.`,
                  };
              }
              return clausula;
          })
...
}, [formData.diaHInicioPago, formData.diaFinPago, formData.inmueble, inmuebleSeleccionado]);
      ...
      setClausulas(prevClausulas => 
          prevClausulas.map(clausula => {
              if (clausula.id === 21) {
                  return {
                      ...clausula,
                      texto: `Cláusula 21: Deudores solidarios: La suscrita ${coarrendatario.first_name}  ${coarrendatario.last_name}, identificada con cedula de ciudadanía no. ${coarrendatario.n_cedula} de ${coarrendatario.c_exp}, con domicilio en la ciudad de  ${coarrendatario.c_vive}, por medio del presente documento me declaro deudora de el arrendador en forma solidaria indivisible junto con el arrendatario, de todas las cargas y obligaciones contenidas en el presente contrato, durante el término inicial como durante sus prórrogas ó renovaciones expresas ó tácitas, por concepto de: arrendamientos, servicios públicos, indemnizaciones, daños en el inmueble, cláusulas penales,  gestión de cobro, costas procesales y cualquier otra derivada del contrato.`,
                  };
              }
              return clausula;
          })
...
const handleSubmit = async (e) => {
  ...
  const dataToSubmit = {
    ...
    clausulas: selectedClausulas.map(clausula => clausula.texto), // Solo los textos de las cláusulas
};

  //console.log('dataToSubmit:', dataToSubmit); // Para depuración
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <select name="coarrendatario" onChange={(e) => handlecoarrendatarioChange(e.target.value)}>
